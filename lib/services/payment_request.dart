// Kept identical to web-wallet/lib/services/payment_request.dart, except for
// the address helpers below (the web wallet uses its AddressBook class).
// Both apps must read and write payment requests the same way.

// Same address rules as AddressbookProvider.
final RegExp _legacyAddress = RegExp(r'^[bB83][1-9A-HJ-NP-Za-km-z]{24,33}$');
final RegExp _bech32Address = RegExp(r'^bs1[a-z0-9]{39,59}$');

bool _isValidAddress(String address) {
  final trimmed = address.trim();
  if (trimmed.isEmpty || trimmed.length > 128) return false;
  return _legacyAddress.hasMatch(trimmed) || _bech32Address.hasMatch(trimmed.toLowerCase());
}
...
///
/// The format of the receive QR code (receive_view.dart), following BIP21.
/// [webLink] wraps it in a link that opens the web wallet's Send tab
/// pre-filled.
